...
class SparseSolverSourceLens(SparseSolverSource):
    ...
    def __init__(self, data_class, lens_model_class, source_model_class, lens_light_model_class,
                 ...
                 k_max=5, n_iter_source=50, n_iter_lens=50, n_weights=1, sparsity_prior_norm=1, force_positivity=True, 
        ...
        super(SparseSolverSourceLens, self).__init__(data_class, lens_model_class, source_model_class, lens_light_model_class=lens_light_model_class,
                                                 ...
                                                 k_max=k_max, n_iter=n_iter_source, n_weights=n_weights)
        self._n_iter_lens = n_iter_lens
    ...
    def _solve(self):
        ...
        for j in range(self._n_weights):
            ...
            prox_g_l = lambda x, y: self.proximal_sparsity_lens(x, y, weights_lens)
            prox_g_s = lambda x, y: self.proximal_sparsity_source(x, y, weights_source)
            ...
            for i_l in range(self._n_iter_lens):
                ...
                grad_f_s = lambda x: self.gradient_loss_source(x, HG)
                ...
                if i_l % 5 == 0 and self._verbose:
                    print("iteration {}-{}-{} : loss = {:.4f}, red-chi2 = {:.4f}, step_diff = {:.2e}"
                          .format(j, i_l, i_s, loss, red_chi2, step_diff))

                if i_l % int(self._n_iter_lens/2) == 0 and self._show_steps:
                    self._plotter.plot_step(S_next, iter_1=j, iter_2=i_l, iter_3=i_s, show_now=True)
            ...
            # update weights
            lambda_source  = self._k_max * self.noise_levels_source_plane
            weights_source = 2. / ( 1. + np.exp(-10. * (lambda_source - alpha_S)) )
            lambda_lens    = self._k_max * self.noise_levels_image_plane
            weights_lens   = 2. / ( 1. + np.exp(-10. * (lambda_lens - alpha_HG)) )
    ...
    def _proximal_sparsity_analysis_lens(self, HG, step, weights):
        """
        ...
        alpha_HG_proxed = proximals.prox_sparsity_wavelets(alpha_HG, step, level_const=level_const, level_pixels=level_pixels,
    ...
    def _proximal_sparsity_synthesis_lens(self, alpha_HG, step, weights):
        """
        returns the proximal operator of the regularisation term
